"""Lumigon - controlled C-axis low-frequency vibration auto-detection test.

This is a real-motion commissioning test.

Behavior:
- Requires C servo ON and Gamma servo OFF.
- Requires zero alarms and validated PR/S-curve configuration.
- Resets C P1-29 auto detection state first.
- Temporarily lowers C P1-30 detection threshold to 300.
- Enables C P1-29 auto detection.
- Executes repeated C-axis motion around the starting point using ONLY
  <= 1.0 degree PR segments.
- Stops early if the drive completes auto detection (P1-29 returns to 0).
- Returns C to the starting feedback position.
- Restores the original C P1-30 value before exit.
- If auto detection is still active at exit, explicitly resets C P1-29 to 0.

Gamma parameters are not modified.
...
def move_relative_segmented(bus: DeltaModbus, delta_degree: float) -> int:
    ...
    while abs(remaining) > 1e-9:
        ...
        p1_29 = bus.read_u16(C_ID, P1_29)
        if p1_29 == 0:
            print("    Auto detection completed (C P1-29 returned to 0).")
...
def main() -> None:
    print("=" * 72)
    print("LUMIGON — C-AXIS MULTI-CYCLE LF AUTO DETECTION")
    print("=" * 72)
    ...
    print(f"Temporary C P1-30 detection threshold: {DETECTION_THRESHOLD}")
    print("Gamma parameters will NOT be modified.")
    ...
    try:
        ...
        confirmation = input(
            "\nType RUN_C_LF_MULTICYCLE exactly to start this real-motion test: "
        ).strip()

        if confirmation != "RUN_C_LF_MULTICYCLE":
            print("Cancelled. No changes or motion issued.")
            return

        print("\nResetting C P1-29 to 0 before test...")
