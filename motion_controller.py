...
class MotionController:
    ...
    def get_current_angle(
        ...
        return self.puu_to_degree(
            axis,
            feedback - self.get_zero(axis),
        )
    ...
    def wait_for_target(
        ...
        while time.monotonic() < deadline:
            ...
            if alarm != 0:
                raise RuntimeError(
                    f"{axis.name}: alarm "
                    f"0x{alarm:04X} during motion."
                )

            last_feedback = self.modbus.read_s32(
                axis.slave_id,
                P0_09,
            )
    ...
    def execute_relative(
        ...
        if (
            ...
            raise RuntimeError(
                f"{axis.name}: internal step "
                f"{delta_degree:+.4f}° exceeds "
    ...
    def move_absolute(
        ...
        while True:
            ...
            if (
                abs(intermediate_target)
                > ABSOLUTE_LIMIT_DEG
            ):
                raise RuntimeError(
                    f"{axis.name}: "
                    "intermediate target "
                    "exceeds software limit."
                )
