...
class P9710MIOLGridWorker(QThread):
    ...
    def __init__(
        ...
        *,
        ...
        self.threshold_lx = float(threshold_lx)
...
def attach_p9710_miol_grid_runtime(window):
    ...
    form.addWidget(QLabel("P-9710 range:"), 0, 0)
    ...
    form.addWidget(QLabel("Trigger threshold:"), 2, 0)
    form.addWidget(threshold_spin, 2, 1)
    ...
    def start():
        ...
        basis = "E-effective (SC)" if spec.flashing else "CW"
        ...
        def on_result(row):
            ...
            if table is not None and point - 1 < table.rowCount():
                ...
                table.setItem(point - 1, 5, QTableWidgetItem(f"{float(row['E_lx']):.3f}"))
                table.setItem(point - 1, 6, QTableWidgetItem(f"{float(row['I_cd']):.1f}"))
                table.setItem(point - 1, 7, QTableWidgetItem(row["basis"]))
